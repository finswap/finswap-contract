...
// CaviarBar with Governance.
contract CaviarBar is ERC20('CaviarBar Token', 'CAVIA'), Ownable {
    // Creates `_amount` token to `_to`. Must only be called by the owner (FinMasterChef).
    ...
    // Safe fins transfer function, just in case if rounding error causes pool to not have enough FINSs.
    function safeFinsTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 finsBal = fins.balanceOf(address(this));
        if (_amount > finsBal) {
            fins.transfer(_to, finsBal);
        } else {
            fins.transfer(_to, _amount);
        }
    }
